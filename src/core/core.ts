...
const twitterRender = (data: TweetApiUtilsData, count: number) => {
  ...
  return async ({handler, output, themeName, themeParam}: twitterRenderParam) => {
    ...
    const replacData = [
      ...
      ['{time-now-yyyy}', new Date().getFullYear().toString()],
      ['{time-now-mm}', (new Date().getMonth() + 1).toString()],
      ['{time-now-dd}', new Date().getDate().toString()],
      ['{time-now-hh}', new Date().getHours().toString()],
      ['{time-now-mi}', new Date().getMinutes().toString()],
      ['{time-now-ss}', new Date().getSeconds().toString()],
      ['{time-tweet-yyyy}', new Date(legacy.createdAt).getFullYear().toString()],
      ['{time-tweet-mm}', (new Date(legacy.createdAt).getMonth() + 1).toString()],
      ['{time-tweet-dd}', new Date(legacy.createdAt).getDate().toString()],
      ['{time-tweet-hh}', new Date(legacy.createdAt).getHours().toString()],
      ['{time-tweet-mi}', new Date(legacy.createdAt).getMinutes().toString()],
      ['{time-tweet-ss}', new Date(legacy.createdAt).getSeconds().toString()],
    ] as [string, string][]
